...
if (isset($_POST['add'])) {
    ...
    set_flash("✅ Đã thêm Department mới");
    header("Location: listdepartments.php"); exit;
}
...
if (isset($_POST['update'])) {
    ...
    set_flash("✅ Đã cập nhật Department");
    header("Location: listdepartments.php"); exit;
}
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $count = $pdo->prepare("SELECT COUNT(*) FROM employees WHERE department_id=:id");
    $count->execute([':id'=>$id]);
    if ($count->fetchColumn() > 0) {
        set_flash("⚠️ Không thể xóa: vẫn còn Employee trong Department này");
    } else {
        $pdo->prepare("DELETE FROM departments WHERE id=:id")->execute([':id'=>$id]);
        set_flash("✅ Đã xóa Department");
    }
    header("Location: listdepartments.php"); exit;
}

// Lấy danh sách
$rows = $pdo->query("SELECT * FROM departments ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
disconnect_db();
?>
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Departments</title><link rel="stylesheet" href="../assets/style.css"></head>
<body>
