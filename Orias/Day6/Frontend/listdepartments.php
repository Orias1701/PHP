...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_SERVER['HTTP_X_REQUESTED_WITH']) || isset($_POST['ajax']))) {
    ...
    if ($a === 'list') {
        $rows = $pdo->query("SELECT id,name FROM departments ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($a === 'add') {
        ...
        $st = $pdo->prepare("INSERT INTO departments(name) VALUES(:n)");
        $st->execute([':n' => $name]);
    ...
    if ($a === 'update') {
        ...
        $st = $pdo->prepare("UPDATE departments SET name=:n WHERE id=:id");
...
<div class="wrap">
    ...
    <button class="btn" onclick="openAdd('departments')" <?= is_admin() ? '' : 'disabled title="Chỉ Admin"' ?>>+ Thêm</button>
    ...
    <table class="table" id="tb">
        ...
        <?php foreach ($rows as $r): ?>
            <tr data-id="<?= $r['id'] ?>">
                ...
                <td>
                    <?php if (is_admin()): ?>
                        <button class="btn" onclick="openEdit(<?= $r['id'] ?>, '<?= safe_html($r['name']) ?>', 'departments')">Sửa</button>
                        <button class="btn" onclick="delRow(<?= $r['id'] ?>, 'listdepartments.php')">Xóa</button>
                    <?php else: ?>
